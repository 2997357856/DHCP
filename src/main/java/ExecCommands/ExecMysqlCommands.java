...
public class ExecMysqlCommands {
    ...
    public static void main(String[] args) throws SQLException {
        ...
        ipList0.add(ip2);
        ipList0.add(ip3);
        EMC.insertIpList(ipList0);
        System.out.println("插入成功");
        List<Ip> ipList1 = EMC.getIpList();
        ipList1.forEach(x-> System.out.println(x));
    }


    //获取数据库中存储的ip集合
    public List<Ip> getIpList() throws SQLException {
        //获取数据库的连接对象
        ...
        //获取结果集的遍历
        ResultSet rs = stmt.executeQuery("select id,ip from pools");
        while (rs.next()){
    ...
    public void insertIp(Ip ip) throws SQLException {
        ...
        GetMysqlConnection GMC = new GetMysqlConnection();
        Connection conn = GMC.getConnection();
        String sql = "insert into pools values(?,?)";
    ...
    //插入多个ip数据
    public void insertIpList(List<Ip> ipList) throws SQLException {
        //获取数据库的连接对象
        GetMysqlConnection GMC = new GetMysqlConnection();
        Connection conn = GMC.getConnection();
        String sql = "insert into pools values(?,?)";
